...
inline float_t dot_qint8_float_cpu(int8_t* a, float_t* b, int64_t n, int64_t ta, int64_t tb, float scale, float zero_point) {
  float_t sum = 0.0;
  int8_t* ai = a;
  float_t* bi = b;
  ...
  for (int64_t i = 0; i < n / BLOCK_SIZE; i++) {
    ...
    __m512 a_sub_f32 = _mm512_sub_ps(a_vec_i8, zero_i8); // float(a) - zero_point
...
Tensor matmul_quant_float_cpu(Tensor a, Tensor b) {
