...
Tensor dequantize_and_unpack(Tensor a, const std::string& typeb_str, unsigned int bits, float scale, float zero_point) {
  at::ScalarType typeb = dtype_string_to_scalar_type(typeb_str);
  at::Device device = a.device();
  if (device.is_cuda()) {
    return dequantize_and_unpack_cuda(a, typeb, bits, scale, zero_point);
  } else if (device.is_cpu()) {
    return dequantize_and_unpack_cpu(a, typeb, bits, scale, zero_point);
  } else {
    TORCH_CHECK(false, "quantize_and_pack not implemented for '", device, "'");
  }
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("dequantize_and_unpack", &dequantize_and_unpack, "Dequantized and Unpack");
